...
3. **削除作業の運用方針**
   **重要**: 不要ファイルは即削除せず、まずアーカイブスへ移動する

### アーカイブス運用
- **場所**: プロジェクトルート直下の `archives/` ディレクトリ
- **目的**: 削除対象のドキュメント・ソースコード両方を一時保管
- **削除判断**: プロジェクト責任者（依頼者）の確認後に実施
...
# 4. 新規作成する場合
# a. アーカイブスディレクトリ作成（なければ）
mkdir -p ./archives/{YYYY-MM-DD}/

# b. 不要ファイルをアーカイブスに移動
mv ./docs/development-flow/requirements/SRS_project-x_2025-07-01.md \
   ./archives/2025-07-14/SRS_project-x_2025-07-01_archived.md
...
### 削除時の安全対策
- **直接削除は原則禁止**（即削除厳禁）
- まずアーカイブス（./archives/）に移動
- 移動時は日付ディレクトリで整理
- アーカイブス内ファイルの最終削除は依頼者判断を仰ぐ
- 誤削除防止のため、削除判断を明示的にチェックする体制
